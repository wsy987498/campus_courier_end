...
express.post("/express_list", async (ctx) => {
  ...
  if (expressName == "all" && sort == "default") {
    let res = await new Promise((resolve, reject) => {
      return db.query(`select * from express_list limit ${(page - 1) * pageSize},${pageSize};`, (err, data) => {
        if (err) throw err
        let obj = {
          code: 200,
          msg: '获取成功！',
    ...
    let res = await new Promise((resolve, reject) => {
      return db.query(`select * from express_list where express_name ='${expressName}' limit ${(page - 1) * pageSize},${pageSize};`, (err, data) => {
        if (err) throw err
        let obj = {
          code: 200,
          msg: '获取成功！',
...
express.post("/add_express", async (ctx) => {
  ...
  const insertExpressSql =
    ...
    (express_name ,express_money,delivery_address,forward_delivery_time,express_type,pick_code,express_recipients,phone,remarks)
    values('${express_name}','${express_money}','${delivery_address}','${forward_delivery_time}','${express_type}','${pick_code}','${express_recipients}','${phone}','${remarks}')`
